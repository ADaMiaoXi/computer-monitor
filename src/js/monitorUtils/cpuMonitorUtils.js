...
const getCPUMaxClockSpeed = function (lines) {
  ...
  const CPUMaxClockSpeedIndex =
    lines.findIndex((line) => line.includes("MaxClockSpeed")) + 1;
...
function getCPUCurrentSpeed(lines, CPUMaxClockSpeed) {
  const processorPerformanceIndex =
    lines.findIndex((l) => l.includes("processor performance")) + 1;

  return `${(
    ((lines[processorPerformanceIndex] / 100) * CPUMaxClockSpeed) /
    1000
  ).toFixed(2)}  GHz`;
}
